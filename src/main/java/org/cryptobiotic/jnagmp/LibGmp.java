 ...
 */
package org.cryptobiotic.jnagmp;
...
public final class LibGmp {
    ...
    private static void loadLibGmp() {
        try {
            ...
            File file = Native.extractFromResourcePath("egkspowm", LibGmp.class.getClassLoader());
     ...
     */
    // ffm:       void __gmpz_import(mpz_ptr, size_t, int, size_t, int, size_t, void*);
    //            void __gmpz_import(MemorySegment x0, long x1, int x2, long x3, int x4, long x5, MemorySegment x6) {
    ...
    // JGMP is more modern jna
    // JGMP     public static native Pointer mpz_export(ByteBuffer rop, SizeTByReference count, int order, SizeT size, int endian,
    //            SizeT nails, MpzT op);
    ...
    // there are no examples that work on arrays
    // void egk_spowm(mpz_t rop, mpz_t* bases, mpz_t* exponents, size_t len, mpz_t modulus);
    // void egk_spowm(MemorySegment rop, MemorySegment bases, MemorySegment exponents, long len, MemorySegment modulus) {
    public static native void egk_spowm(mpz_t rop, Pointer bases, Pointer exponents, long len, mpz_t modulus);
