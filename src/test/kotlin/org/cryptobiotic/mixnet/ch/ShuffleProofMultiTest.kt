...
fun expectProofMulti(nballots:Int, width: Int): String {
...
fun expectCheckMulti(nballots:Int, width: Int): String {
    val N = nballots*width
...
class ShuffleProofMultiTest {
    ...
    fun runShuffleProof(nrows: Int, width: Int, group: GroupContext, showExps: Boolean = true, showTiming: Boolean = true) {
        ...
        if (showExps) println("  after shuffleProof: ${group.showAndClearCountPowP()} ${expectProofMulti(nrows, width)}")
        ...
        if (showExps) println("  after checkShuffleProof: ${group.showAndClearCountPowP()} ${expectCheckMulti(nrows, width)}")
        assertTrue(valid)
